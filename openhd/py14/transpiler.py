...
from ..jit import HYPERVECTOR_TYPE, HYPERMATRIX_TYPE, ARG_PREFIX


TAG = "py14.transpiler"

def transpile_cuda_from_ast(tree, var_types):
    """
    Transpile python ast into CUDA C code
    """
    ...
    transpiler = CppTranspiler(var_types)
...
def generate_template_fun(node, body, var_types):
    params = []
    for arg in node.args.args:
        if get_id(arg) not in var_types:
            debug.log(debug.ERROR, TAG, "Unknown type: " + get_id(arg))
            debug.log(debug.ERROR, TAG, "From : " + str(var_types))
            assert(False)
        
        item_type = var_types[get_id(arg)]
        if not isinstance(item_type, str):
            item_type = item_type.__name__

        params.append((item_type, get_id(arg)))
...
#template = "inline "
    ...
    return_type = "extern \"C\" __global__ void"

    funcdef = "{1} {2}({3})".format(template, return_type, node.name,
                                          ", ".join(params))
    return funcdef + " {\n" + body + "\n}"
...
class CppTranspiler(CLikeTranspiler):
    ...
    def visit_FunctionDef(self, node):
        ...
        return generate_template_fun(
                node, body, self.var_types)
        # else:
    ...
    def visit_Call(self, node):
        ...
        if fname == "int":
            return "__to_int__({0})".format(args)
        elif fname == "float":
            return "__to_float__({0})".format(args)
        elif fname == "bool":
            return "__to_bool__({0})".format(args)
        elif fname == "str":
    ...
    def visit_For(self, node):
        ...
        it = self.visit(node.iter)
        if sys.version_info[0] >= 3:
            assert("rangepp::xrange" in it)
            it = it.replace("rangepp::xrange(", "")
        else:
            assert("rangepp::range" in it)
            it = it.replace("rangepp::range(", "")
        it = it[:len(it)-1]

        buf = []
        buf.append('#pragma unroll 1')
        buf.append('for(int {0}=0; {1}<{2}; ++{3}) {{'.format(
    ...
    var_type_map = {
            HYPERMATRIX_TYPE: "float",
            HYPERVECTOR_TYPE: "float",
            int: "int",
            float: "float",
            bool: "bool",
            }
    ...
    def visit_Assign(self, node):
        ...
        if isinstance(node.scopes[-1], ast.If):
            ...
            if isinstance(target, ast.Name) and \
                    target.id in outer_if.common_vars:
                value = self.visit(node.value)
        ...
        if isinstance(target, ast.Call):
            ...
            if (isinstance(target, ast.Name) and
                ...
                typestr = 'auto'
                if target in self.var_types:
                    if self.var_types[target] in self.var_type_map:
                        typestr = self.var_type_map[self.var_types[target]]

                return "{0} {1} = {2};".format(typestr, target, value)
